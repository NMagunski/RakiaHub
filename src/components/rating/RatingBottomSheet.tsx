...
export default function RatingBottomSheet({ onClose, preselected }: Props) {
  ...
  // Lock page scroll while sheet is open so slider drags don't move the feed behind
  useEffect(() => {
  ...
  async function handleQuickSave() {
    ...
    try {
      const body =
        selected.kind === "existing"
          ...
          : {
              ...
              personal_producer: selected.producer,
              personal_fruit: selected.fruit,
            };
      ...
      const { id: ratingId } = await res.json();
  ...
  async function handleAddDetails() {
    ...
    try {
      ...
      window.dispatchEvent(new Event("rating-saved"));
      onClose();
      router.push(`/rate/${ratingId}/details`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Грешка");
      setSaving(false);
    }
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div className="fixed bottom-0 left-0 right-0 z-50 rounded-t-3xl bg-background px-5 pb-10 pt-4 shadow-2xl animate-slide-up">
        {/* Handle */}
        <div className="mx-auto mb-4 h-1 w-10 rounded-full bg-accent/30" />
