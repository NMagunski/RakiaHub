...
export default function BottomNav() {
  ...
  return (
    <nav
      ...
      style={{
        background: "rgba(240,223,192,0.80)",
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center gap-1 px-4 py-1.5 rounded-xl transition-all ${
                active ? "text-walnut" : "text-accent/70"
              }`}
              style={active ? {
                background: "rgba(122,82,48,0.10)",
                boxShadow: "0 2px 10px rgba(122,82,48,0.15)",
              } : {}}
            >
              {tab.icon(active)}
              {tab.badge && pendingCount > 0 && (
                <span className="absolute top-0 right-1 flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-bold text-cream" style={{ background: "linear-gradient(135deg,#7A5230,#3B2008)" }}>
                  {pendingCount > 9 ? "9+" : pendingCount}
                </span>
              )}
              <span className={`text-[10px] font-medium ${active ? "text-walnut" : "text-accent/60"}`}>{tab.label}</span>
            </Link>
          );
        })}
      </div>
